...
**進め方は2026-08-22に決まった。二段階である。**

1. **先に止血する。** 承諾していない3箇所（一海のALLIANCE節、法人名を含む沿革、住所の番地）を管理画面から消す。田中氏が行う（`docs/live-site-takedown.md`）
2. そのうえで、WordPress + SWELL 上で作り直す

**ただしCodexの作業内容は変わらない。** 素の HTML と CSS で3ページを書く。
WordPressへ組み込むのは田中氏の側の作業であり、各指示書の設計（書体・配色・禁止事項・原稿・構成）はそのまま使える。
...
**フレームワークを挟まない素の HTML と CSS で書く。** そのまま開いて見られる形であることが、
田中氏が中身を確かめ、WordPress + SWELL へ移すときの前提になる（`docs/open-questions.md` §0）。
